...
function ProgramCard({ program, color }) {
  return (
    <div className="col-md-6 col-lg-3 mb-4">
      <div className={`card h-100 border-${color} shadow-sm`}>
        <div className="card-body">
          <h5 className={`card-title text-${color}`}>{program.name}</h5>
          <div className="mb-2">비용: <span className="fw-bold">{program.total_price.toLocaleString()}원</span></div>
          <div className="mb-2">기간: {program.total_week}주</div>
        </div>
      </div>
    </div>
  );
...
export default function MainPage() {
  const active = programs.filter(p => p.status === 1);
  const pending = programs.filter(p => p.status === 0);
  ...
  return (
    <div className="container py-4">
      ...
      <div className="row mb-5">
        {active.length ? active.map(p => <ProgramCard key={p.id} program={p} color="primary" />) : (
      ...
      <div className="row">
        {pending.length ? pending.map(p => <ProgramCard key={p.id} program={p} color="secondary" />) : (
