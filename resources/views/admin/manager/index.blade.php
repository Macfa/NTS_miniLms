...
<div class="container mx-auto p-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
        ...
        <div class="w-full md:w-1/3 relative">
            <input type="text" name="searchManagerKeyword" placeholder="이름 또는 이메일 검색..." class="w-full pl-10 pr-4 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <tr>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <input type="checkbox" class="row-check" value="{{ $manager->user_id }}">
                      </td>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-green-500">{{ $manager->user->status }}</td>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const searchInput = document.querySelector('input[name="searchManagerKeyword"]');
        ...
        searchInput.addEventListener('input', async function () {
            ...
            if (keyword.length > 0) {
                try {
                    const url = `{{ route('admin.manager.search') }}?keyword=${encodeURIComponent(keyword)}`;
                    ...
                    managers.forEach((manager, idx) => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-green-500">${manager.user.status}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="/admin/manager/${manager.id}/edit" class="text-indigo-600 hover:text-indigo-900">수정</a>
                            </td>
                        `;
