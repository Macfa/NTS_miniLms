...
class CourseAggregate
{
    ...
    public function __construct(Course $courseModel, MediaService $mediaService)
    {
     ...
     * @return Course
     */
    public function createCourseWithMedia(array $data, array $files, User $user): Course
    {
        $Course = DB::transaction(function () use ($data, $files, $user) {
            $payload = $data;
            ...
            // 미디어 저장
            $this->mediaService->storeMedia($Course, $files);

            return $Course;
        });

        // 트랜잭션 커밋 후 이벤트 발행
        StoreCourseEvent::dispatch($Course);
