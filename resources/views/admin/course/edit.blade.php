...
<div class="container mx-auto p-8">
  ...
  <div class="bg-white rounded-lg shadow-md p-6">
    <form action="{{ route('admin.Course.update', $Course->id) }}" method="POST" id="CourseForm">
      @csrf
      @method('PUT')
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <select name="manager_id" class="form-select" required>
          ...
          <option value="{{ $manager->id }}" @if(old('manager_id', $Course->manager_id) == $manager->id) selected @endif>
      ...
      <div class="mb-3">
        ...
        <input type="number" name="total_week" class="form-control" value="{{ old('total_week', $Course->total_week) }}">
        @error('total_week')
        <div class="text-danger small">{{ $message }}</div>
        @enderror
      </div>
      <div class="mb-3">
        ...
        <input type="number" name="limit_count" class="form-control" value="{{ old('limit_count', $Course->limit_count) }}">
        @error('limit_count')
        <div class="text-danger small">{{ $message }}</div>
        @enderror
      </div>
      <div class="mb-3">
        ...
        <input type="number" name="total_price" class="form-control" value="{{ old('total_price', $Course->total_price) }}">
        @error('total_price')
        <div class="text-danger small">{{ $message }}</div>
        @enderror
      </div>
      <div class="mb-3">
        ...
        <select name="status" class="form-select">
          <option value="1" @if(old('status', $Course->status)=="1") selected @endif>활성</option>
          <option value="0" @if(old('status', $Course->status)=="0") selected @endif>비활성</option>
        </select>
        @error('status')
        <div class="text-danger small">{{ $message }}</div>
        @enderror
