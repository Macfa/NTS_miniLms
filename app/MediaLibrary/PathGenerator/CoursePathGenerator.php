...
/**
 * Course 전용 PathGenerator
 * 기본(미디어 ID 기반) 디렉토리 구조를 모델/컬렉션 기반 구조로 바꿔
 * 한 번의 다중 업로드(또는 동일 Course에 대한 반복 업로드) 파일이
 * 하나의 폴더(Course/{Course_id}/{collection})에 모이도록 한다.
 *
 * 구조 예시:
 *  Course/15/images/파일들
 *  Course/15/images/conversions/
 *  Course/15/images/responsive/
 *  Course/15/videos/...
 */
class CoursePathGenerator implements PathGenerator
{
    ...
    {
        $model = $media->model; // Course 인스턴스
        // Course 외에 잘못 매핑되었을 경우를 대비한 fallback
        $short = strtolower((new \ReflectionClass($model))->getShortName());
        // Course/{Course_id}/{collection_name}/
